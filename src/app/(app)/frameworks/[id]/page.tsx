...
export default function FrameworkPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!fw) {
    return (
      <div className="max-w-2xl mx-auto text-center py-24">
        ...
        <Link href="/frameworks" className="text-violet-400 hover:text-violet-300 text-sm">
          ← 一覧に戻る
        </Link>
      </div>
    )
  ...
  const handleSave = () => {
    ...
    const contentHtml = fw.steps
      .map(
        (s, i) =>
          `<p><strong>Q${i + 1}. ${s.question}</strong></p><p>${answers[i].replace(/\n/g, '<br>')}</p>`
      )
      .join('')

    const plainText = fw.steps
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-zinc-500">
            ステップ {step + 1} / {fw.steps.length}
          </span>
          <span className="text-xs text-zinc-600">
            {Math.round(((step + 1) / fw.steps.length) * 100)}%
          </span>
        </div>
        ...
        <div className="flex gap-1.5 mt-3">
          {fw.steps.map((_, i) => (
            <button
              ...
              onClick={() => {
                // 入力済みのステップへのみジャンプ可
                if (i <= step || answers[i - 1]?.trim()) setStep(i)
              }}
              ...
              style={{
                backgroundColor: i <= step ? meta.color : '#3f3f46',
              }}
            />
          ))}
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="mb-6"
        >
          <h2 className="text-white text-lg font-medium leading-relaxed mb-3">
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {isLast ? (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 bg-violet-600 hover:bg-violet-500 disabled:bg-zinc-800 disabled:text-zinc-600 text-white text-sm font-medium rounded-lg transition-colors"
          >
            <Check className="w-4 h-4" />
            {saving ? '保存中…' : 'エントリとして保存'}
          </button>
        ) : (
          <button
            onClick={handleNext}
            disabled={!canNext}
            className="flex items-center gap-2 px-5 py-2 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-30 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors"
          >
            次へ
