...
export default function FrameworksPage() {
  ...
  const sessionCounts = Object.fromEntries(
    frameworks.map((fw) => [fw.id, entries.filter((e) => e.frameworkId === fw.id).length])
  )
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-zinc-500 text-sm mt-1">思考を深めるメンタルモデルを選んで、ガイド付きジャーナリングを始めよう</p>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {visible.map((fw, i) => {
          ...
          const count = sessionCounts[fw.id] ?? 0
          return (
            <motion.div
              key={fw.id}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="inline-block text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: meta.bg, color: meta.color }}>
                      {meta.label}
                    </span>
                    {count > 0 && (
                      <span className="flex items-center gap-1 text-xs text-emerald-400">
                        <CheckCircle2 className="w-3 h-3" />{count}回実施済み
                      </span>
                    )}
                  </div>
                  <h2 className="text-white font-semibold leading-snug mb-1">{fw.name}</h2>
                  <p className="text-zinc-500 text-xs mb-2">{fw.nameEn}</p>
                  <p className="text-zinc-400 text-sm leading-relaxed line-clamp-2">{fw.description}</p>
                </div>
              ...
              <div className="flex items-center gap-3 mt-4 pt-3 border-t border-zinc-800">
                <span className="text-xs text-zinc-600">{fw.steps.length} ステップ</span>
                <span className="text-zinc-800">·</span>
                <span className="text-xs text-zinc-500 italic">{fw.purpose}</span>
              </div>
            </motion.div>
          )
        })}
