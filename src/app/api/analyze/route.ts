...
/**
 * JSON文字列リテラルの内側にある制御文字（\x00-\x1F）を
 * 正しいJSONエスケープに置換する。
 * 文字単位のステートマシンで処理するため、正規表現では対応できない
 * 複数行にまたがるケースや任意の制御文字も確実に処理できる。
 */
function escapeControlCharsInJsonStrings(json: string): string {
  ...
  while (i < json.length) {
    ...
    if (!inString) {
      ...
      if (ch === '\\') {
        // エスケープシーケンス：バックスラッシュと次の1文字をそのまま通す
        result += ch
        i++
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // LLMがJSON文字列リテラル内に生の制御文字を出力した場合のフォールバック
    analysis = JSON.parse(escapeControlCharsInJsonStrings(jsonStr))
  }
